    ...
    static function uploadFiles(array $parametersOfUpload = array())
    {
        ...
        if ($result = $bfu->handleUpload()) {
            ...
            die(json_encode(array(
                'OK' => 0,
                'error' => array(
                    'code' => $bfu->getErrorCode(),
                    'message' => $bfu->getErrorMessage()
                )
            )));
...
//                'target_dir' => false,
    ...
    protected function writeUploadTo($file_path, $file_data_name = false, $mode = 'wb')
    {
        ...
        if (!file_exists($base_dir) && !mkdir($base_dir, 0777, true) && !is_dir($base_dir)) {
